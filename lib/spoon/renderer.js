...
Renderer.prototype.addSlot = function addSlot(ast, until, push) {
  var slot = { ast: ast, until: until };
...
Renderer.prototype.renderDo = function renderDo(args, instr) {
  var slot = [];
  this.addSlot(slot, instr.until);
  return ['do', this.renderInstruction(instr.test), ['block', slot]];
};
