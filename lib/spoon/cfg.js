...
Cfg.prototype.getLevel = function getLevel(level) {
  var map = {},
      children = {},
      levels = [ [] ],
      left = this.roots.length;

  this.roots.forEach(function(root) {
    map[root.id] = root;
    if (root.root.parent) {
      if (!children[root.root.parent.id]) children[root.root.parent.id] = [];
      children[root.root.parent.id].push(root);
    } else {
      levels[0].push(root);
      left--;
    }
  ...
  for (var i = 1; i <= level && left > 0; i++) {
    levels.push([]);
    levels[i - 1].forEach(function(root) {
...
Cfg.prototype.asyncify = function asyncify(ast, level) {
  var rootCount = this.roots.length,
      roots = this.getLevel(level);
  ...
  var target = ast.body[0].expression;
