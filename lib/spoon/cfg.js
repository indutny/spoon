...
Cfg.prototype.split = function split(at, asyncify) {
  ...
  info.next.frontier.forEach(function(block) {
    var fn = this.split(block.instructions[0]);

    // Reinsert function into one of predecessors
    var pred = block.predecessors[0];
    if (pred) {
      pred.instructions.push(fn, pred.instructions.pop());
      fn.block = pred;
    }

    block.predecessors.forEach(function(pred) {
      assert.equal(pred.successors.length, 1);
      // Remove goto
...
Cfg.prototype.asyncify = function asyncify(fn) {
  ...
  for (var i = 0; i < this.blocks.length; i++) {
    ...
    for (var j = 0; j < block.instructions.length; j++) {
      ...
      if (instr.type !== 'call') continue;
      var name = instr.args[0];
      if (name.type !== 'get' || name.args[0] !== fn) continue;

      // Split graph at instruction
      this.split(instr, true);
  ...
  var roots = (rootCount > 2 ? [ this.roots[1] ] : [])
                  .concat(this.roots.slice(rootCount));
  ...
  roots.forEach(function(root) {
    root.exits.forEach(function(block) {
      ...
      if (last.type === 'async-return' || last.type === 'async-goto') return;
      if (last.type === 'fn' && /^__\$fn/.test(last.name)) return;
...
Cfg.prototype.visitDoWhile = function visitDoWhile(ast) {
  return this.enterLoop(function(end) {
    ...
    var instr = this.current.branch('do', [], pre, extra);
    ...
    var test = this.visit(ast.test);
    instr.addArg(test);
    this.current.branch('doend', [ test ], next, end);
