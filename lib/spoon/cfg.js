...
Cfg.prototype.split = function split(pos) {
  var block = pos.block,
      index = block.instructions.indexOf(pos),
      head = block.instructions.slice(0, index),
      tail = block.instructions.slice(index);
  ...
  block.successors.forEach(function(succ) {
    succ.predecessors.splice(succ.predecessors.indexOf(block), 1, next);
  ...
  // Traverse blocks starting from next, split on every join point start
};

Cfg.prototype.asyncify = function asyncify() {
