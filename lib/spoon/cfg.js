...
Cfg.prototype.visitLogical = function visitLogical(ast) {
  ...
  this.add('logical', [ left ]);
  this.current.addSuccessor(tblock);
  this.current.addSuccessor(fblock);
  this.current.end();
...
Cfg.prototype.visitConditional = function visitConditional(ast) {
  ...
  var instr = this.add('ternary', [this.visit(ast.test), tblock, fblock]);
...
Cfg.prototype.visitArray = function visitObject(ast) {
...
Cfg.prototype.visitForIn = function visitFor(ast) {
  ...
  return this.enterLoop(function(end) {
    ...
    var instr = this.add('forin', [this.visit(left), this.visit(ast.right)]);
...
Cfg.prototype.visitTry = function visitTry(ast) {
  ...
  var instr = this.add('try');

  this.current.addSuccessor(body);
  this.current.addSuccessor(caught);
  this.current.end();
